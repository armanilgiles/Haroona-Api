...
BRAND_LOGO_URL = None  # keep existing logo if the brand already exists
...
PRODUCTS = [
{
    ...
    "image_url": 'https://cdn.shopify.com/s/files/1/0735/4893/products/tai-one-shoulder-ribbed-seamless-top-top-mure-grand-black-813557.jpg?v=1697938537',  # replace later with CDN image URL
    "category": "tops",
...
def get_or_create_city(db, country_id: int) -> City:
    ...
    if city:
        ...
        city.latitude = Decimal("40.712776")
        city.longitude = Decimal("-74.005974")
        if not city.marker_color:
            city.marker_color = "#D97706"
        return city
...
def upsert_normalized_row(db, raw: AwinProductFeedRaw, item: dict) -> AwinProductNormalized:
    ...
    row.review_notes = "Manually curated Mure + Grand NYC products."
...
def main() -> None:
    ...
    try:
        ...
        db.commit()
        print(f"Added/updated {len(created_or_updated)} NYC Mure + Grand products:")
